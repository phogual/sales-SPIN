    ...
    return (
        <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex flex-col gap-3 overflow-y-auto pr-2 custom-scrollbar flex-1">
                {(questions || []).map((q, i) => (
                    <div key={i} className="bg-white/5 border-white/5 p-4 rounded-2xl border shrink-0 transition-colors hover:bg-white/10">
...
export const AnalysisView: React.FC<AnalysisViewProps> = ({ result, onReset, mode }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Sales_Diagnosis_${editableName}_${mode || 'standard'}.pdf`);
      ...
      <PageWrapper index={0}>
        <div className="flex justify-between items-start mb-10">
            <div className="flex flex-col">
                <h1 className="text-[58px] font-black text-white leading-[0.9] tracking-tighter uppercase">Sales <span className="text-indigo-500">Diagnosis</span></h1>
                <div className="h-1 w-20 bg-indigo-500 mt-4 shadow-[0_0_10px_#6366f1]"></div>
                <span className="text-[12px] font-black text-indigo-400 uppercase tracking-[0.4em] mt-2 italic">세일즈 인텔리전스 엔진 V2.2</span>
            </div>
            <div className="text-right pt-2 pr-1">
                ...
                <div className="text-2xl font-black text-white italic tracking-tight decoration-indigo-500/50 underline-offset-8 underline">{editableName}님</div>
            </div>
        </div>
        <div className="grid grid-cols-12 gap-10 flex-1 items-center">
            <div className="col-span-5 flex flex-col items-center justify-center">
                <div className="relative w-[300px] h-[300px] flex items-center justify-center">
                    <div className="absolute inset-0 rounded-full border-[25px] border-indigo-500/10 shadow-inner"></div>
                    <div className="absolute inset-0 rounded-full border-[25px] border-indigo-500 border-t-transparent border-r-transparent transform -rotate-45 shadow-lg"></div>
                    <div className="flex flex-col items-center z-10">
                        ...
                        <div className="bg-indigo-600 px-8 py-1.5 rounded-full mt-2 shadow-lg transition-colors"><span className="text-[12px] font-black text-white uppercase tracking-widest italic">SPIN SCORE</span></div>
                    </div>
                </div>
            </div>
      ...
      <PageWrapper index={5}>
        <div className="flex flex-col h-full bg-[#0b1018] border-emerald-500/20 rounded-[40px] border-2 p-12 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col gap-8">
                <div className="flex items-center gap-4">
                    <div className="w-2 h-8 bg-emerald-500 rounded-full shadow-[0_0_12px_#10b981]"></div>
